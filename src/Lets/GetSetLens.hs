...
setL k = Lens setter getter
  where
    setter set' bool = case bool of
      True -> Set.insert k set'
      False -> Set.delete k set'
    getter = Set.member k
...
compose ::
  ...
  Lens a b ->
  Lens a c
compose (Lens setter1 getter1) (Lens setter2 getter2) =
  Lens setter getter
  where
    setter a c =
      let ba = setter2 a
...
product ::
  ...
  Lens c d ->
  Lens (a, c) (b, d)
product (Lens setter1 getter1) (Lens setter2 getter2) =
  Lens setter getter
  where
    setter (a, c) (b, d) = (setter1 a b, setter2 c d)
    getter (a, c) = (getter1 a, getter2 c)
...
choice ::
  ...
  Lens b x ->
  Lens (Either a b) x
choice (Lens setter1 getter1) (Lens setter2 getter2) =
  Lens setter getter
  where
    setter either' x = case either' of
      Left a -> Left (setter1 a x)
      Right b -> Right (setter2 b x)
    getter either' = case either' of
      Left a -> getter1 a
      Right b -> getter2 b
